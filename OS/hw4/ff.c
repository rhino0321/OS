...
char* malloc(size_t alloc_size)
{
    ...
    while (start->free == 0 || start->size < 32 * count)
    {
        start = start->next;
    }
    if (start->size - 32 * count < 32)
    {
        start->size = 32 * count;
        start->free = 0;
    }
    ...
    {
        struct block* next_header = (struct block*)((char *)start + 32 + 32 * count);
        next_header->size = (start->size - 32 * count - 32);
        next_header->free = 1;
        next_header->prev = start;
        next_header->next = start->next;
        start->size = 32 * count;
        start->free = 0;
        start->next = next_header;
    }
    return (char *)start + 32;
}
